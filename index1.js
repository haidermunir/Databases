...
app.post('/customers', (req, res) => {
    let cus = req.body;
    var sql = "SET @customer_id = ?;SET @name = ?;SET @branch_id = ?;SET @password = ?;SET @city = ?;SET @phone = ?; SET @cnic = ?;SET @last_login = ?; \
    CALL CustomersAddOrEdit(@customer_id, @name, @branch_id, @password, @city, @phone, @cnic, @last_login);";
    mysqlConnection.query(sql, [cus.customer_id, cus.name, cus.branch_id, cus.password, cus.city, cus.phone, cus.cnic, cus.last_login], (err, rows, fields) => {
        if (!err)
...
app.get('/login:id',(req, res)=>{
    let usr = req.body;
    var sql = `SELECT * FROM customer_details WHERE customer_id = \'${usr.customer_id}\' AND password = \'${usr.password}\'`;
    mysqlConnection.query(sql, (err, res) => {
        if (!err)
